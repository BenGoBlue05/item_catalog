...
engine = create_engine('sqlite:///garagesale.db', connect_args={'check_same_thread': False})
...
def add_garage_sale():
    ...
    if request.method == 'POST':
        sale = GarageSale(name=request.form['name'], address=request.form['address'], user_id=login_session['user_id'])
        session.add(sale)
        session.commit()
...
@app.route('/garagesales/<int:garage_sale_id>/delete/', methods=['GET', 'POST'])
...
def show_garage_sale_details(garage_sale_id):
    sale = session.query(GarageSale).filter_by(id=garage_sale_id).one()
    items = session.query(Item).filter_by(garage_sale_id=garage_sale_id).all()
    return render_template('garage_sale_details.html', garage_sale=sale, products=items)


@app.route('/garagesales/<int:garage_sale_id>/items/new', methods=['GET', 'POST'])
...
@app.route('/garagesales/<int:garage_sale_id>/items/<int:item_id>/edit/', methods=['GET', 'POST'])
def edit_item(garage_sale_id, item_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('edit_item.html', garage_sale_id=garage_sale_id, item=item)


@app.route('/garagesales/<int:garage_sale_id>/items/<int:item_id>/delete/', methods=['GET', 'POST'])
def delete_item(garage_sale_id, item_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('delete_item.html', garage_sale_id=garage_sale_id, item=item)
...
def create_user():
    user = User(name=login_session['username'], email=login_session['email'], img_url=login_session['picture'])
    session.add(user)
    session.commit()
